...
function saveSurveyToCSV(survey) {
  ...
  const filename = path.join(
    surveysDir,
    `survey_${survey.userId}_${timestamp}.csv`,
  );
...
client.once("ready", () => {
  ...
  webformBotChannel = client.channels.cache.find(
    (ch) => ch.name === "webform-bot",
  );

  if (webformBotChannel) {
    console.log("Found webform-bot channel:", webformBotChannel.id);
    console.log(
      "Message listener is active for channel:",
      webformBotChannel.name,
    );
  } else {
  ...
  if (!message.guild && !message.author.bot) {
    ...
    if (!validationResult.isValid) {
      try {
        ...
        await message.reply(
          `${validationResult.error} ${validationResult.details.expected}`,
        );
      }
      return;
    }
    ...
    try {
      ...
      if (survey.currentQuestion >= survey.questions.length) {
        ...
        if (survey.webhookUrl) {
          try {
            ...
            if (!webhookSuccess) {
              console.error(
                "Failed to deliver webhook for user:",
                message.author.id,
              );
      ...
      const formatInst = getFormatInstructions(
        nextQuestion.format,
        nextQuestion.options,
        nextQuestion.question.toLowerCase(),
      );
      const fullMessage = formatInst
        ? `${nextMessage} ${formatInst}`
        : nextMessage;
    ...
    if (!/^[a-zA-Z\s'-]+$/.test(response)) {
      return {
        ...
        details: {
          provided: response,
          expected:
            "A name using only letters, spaces, hyphens, or apostrophes",
          issues: [
            /[0-9]/.test(response) ? "Contains numbers" : null,
            /[^a-zA-Z\s'-]/.test(response)
    ...
    case "text":
      return {
        ...
        details: {
          ...
          issues: [response.length === 0 ? "Empty response" : null].filter(
            Boolean,
          ),
        },
      };
    case "multiple":
      ...
      return {
        ...
        details: {
          ...
          issues: [
            !isValidOption
              ? `"${response}" is not one of the valid options`
              : null,
            `Valid options are: ${Object.entries(options)
  ...
  try {
    ...
    if (!client.isReady()) {
      return res.status(503).json({
        ...
        details:
          "Discord bot is not connected. Please check the server logs for details.",
      });
    }
    ...
    if (!webformBotChannel) {
      return res.status(404).json({
        ...
        details:
          'The webform-bot channel does not exist. Please create a channel named "webform-bot".',
      });
    }
    ...
    const botResponse = await new Promise((resolve) => {
      ...
      console.log(
        "Added handler to map. Active handlers:",
...
app.post("/api/dm", async (req, res) => {
  try {
    const { userId, message, isStart, questions, format, options, webhookUrl } =
      req.body;
      ...
      if (isStart) {
        ...
        try {
          ...
          const formatInst = getFormatInstructions(
            ...
            firstQuestion.question.toLowerCase(),
          );
          const fullMessage = formatInst
            ? `${welcomeMessage} ${formatInst}`
            : welcomeMessage;
